<template>
  <div>
    ...
    <div class="challengeContainer">
      ...
      >Giv udfordringen et navn</h2>

      <!-- H1 for everything else: -->
      <h1 class="hidden-lg-and-up smallH1">Lav Udfordring</h1>
      <h2 class="text-xs-center hidden-lg-and-up smallH1 teal--text mb-2">Giv udfordringen et navn</h2>
    </div>
    ...
    <v-layout align-center justify-center row fill-heigh>
      <v-flex xs12 md7>
        <v-form>
          ...
          <v-text-field v-model="udfordringNavn" required label="Navn på udfordring"></v-text-field>
          <!-- header with Beskrivelse: -->
          <h4 class="mb-2">Beskrivelse:</h4>
          <!-- textbox for the description-->
          <v-textarea outline auto-grow label="Beskriv din udfordring" v-model="udfordringBeskriv"></v-textarea>

          <div class="btnContainer">
            <v-btn
              class="startChalBtn"
              :to="{name: 'CreateTrin1', params: {user_id: this.$route.params.user_id}}"
...
export default {
  ...
  data() {
    return {
      udfordringNavn: null,
      udfordringBeskriv: null
    };
  },
  methods: {
    // onclick event on button that creates name and description
    // if the name and description has been made you can update
  },
  created() {
